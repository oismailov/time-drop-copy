...
func InitFriend(r *mux.Router) {
	l4g.Debug("Initializing v1 friends api routes")
	friendsController := FriendsCtrl{}
	r.Handle("/friends", api.ApiTokenRequired(friendsController.List)).Methods("GET")
	r.Handle("/friends", api.ApiTokenRequired(friendsController.AddFriend)).Methods("POST")
	r.Handle("/friends/{friendID:[0-9]+}", api.ApiTokenRequired(friendsController.RemoveFriend)).Methods("DELETE")
	r.Handle("/friends/request", api.ApiTokenRequired(friendsController.SendFriendRequest)).Methods("POST")
	r.Handle("/friends/request", api.ApiTokenRequired(friendsController.ListFriendRequest)).Methods("GET")
	r.Handle("/friends/request/{friendID:[0-9]+}", api.ApiTokenRequired(friendsController.DeclineFriendRequest)).Methods("DELETE")
}
...
func (friendsCtrl FriendsCtrl) DeclineFriendRequest(res http.ResponseWriter, req *http.Request) {
	...
	friendRequestID := strings.Replace(req.URL.String(), "/api/v1/friends/request/", "", 1)
...
func (friendsCtrl FriendsCtrl) RemoveFriend(res http.ResponseWriter, req *http.Request) {
	r := render.New(render.Options{})
	friendID := strings.Replace(req.URL.String(), "/api/v1/friends/", "", 1)
