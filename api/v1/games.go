...
func InitGames(r *mux.Router) {
	l4g.Debug("Initializing v1 games api routes")
	gamesController := GameCtrl{}
	r.Handle("/games", api.ApiTokenRequired(gamesController.List)).Methods("GET")
	r.Handle("/games/history", api.ApiTokenRequired(gamesController.ListHistory)).Methods("POST")
	r.Handle("/games", api.ApiTokenRequired(gamesController.Create)).Methods("POST")
	r.Handle("/games/{gameID:[0-9]+}/start", api.ApiTokenRequired(gamesController.StartGame)).Methods("POST")
	r.Handle("/games/{gameID:[0-9]+}/result", api.ApiTokenRequired(gamesController.Result)).Methods("POST")
}
...
func (gameCtrl GameCtrl) StartGame(res http.ResponseWriter, req *http.Request) {
	...
	gameString := strings.Replace(req.URL.String(), "/api/v1/games/", "", 1)
	gameID := strings.Replace(gameString, "/start", "", 1)
...
func (gameCtrl GameCtrl) Result(res http.ResponseWriter, req *http.Request) {
	...
	gameString := strings.Replace(req.URL.String(), "/api/v1/games/", "", 1)
	gameID := strings.Replace(gameString, "/result", "", 1)

	///games/{gameID:[0-9]+}/result
